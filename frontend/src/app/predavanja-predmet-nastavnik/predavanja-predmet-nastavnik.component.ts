...
@Component({
  selector: 'app-predavanja-predmet-nastavnik',
  templateUrl: './predavanja-predmet-nastavnik.component.html',
  styleUrls: ['./predavanja-predmet-nastavnik.component.css']
})
export class PredavanjaPredmetNastavnikComponent implements OnInit {
  ...
  ngOnInit(): void {
    ...
    this.servis.dohvMaterijaleZaNastavu(this.user.username, "predavanje").subscribe((mm: Materijali[]) => {
  ...
  add() {
    ...
    if (!this.predmet) {
      ...
      this.servis.dodajMaterijal(this.predmet, "predavanje", this.slika, this.velicina, this.user.username, "").subscribe();
